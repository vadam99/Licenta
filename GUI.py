...
genereaza_tabla()
copie_tabla_insert = [[tabla[i][j] for j in range(len(tabla[0]))] for i in range(len(tabla))]  # Folosita pentru update la user_insert
...
def draw_solutie():
    ...
    for i in range(9):
        for j in range(9):
            if tabla[i][j] == 0:  # Dorim ca numerele care au fost initial pe tabla de joc sa ramana colorate cu "darkgreen", iar cele inserate de solver sa fie "red".
                number = font.render(str(copie_tabla[i][j]), True, pygame.Color("red"))  # Punem solutia din copie_tabla
                screen.blit(number, pygame.Vector2((j * n) + offset, (i * n) + offset - 1000 // 166.66))
            else:
                number = font.render(str(tabla[i][j]), True, pygame.Color("darkgreen"))
                screen.blit(number, pygame.Vector2((j * n) + offset, (i * n) + offset - 1000 // 166.66))
    ...
    for event in pygame.event.get():
        ...
        if startedPlaying == True:  # Daca am ales sa dam pe Play
            ...
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:  # Verificam daca e apasat click stanga
                ...
                if 0 <= i < 9 and 0 <= j < 9 and tabla[i][j] == 0:  # Daca pozitia din tabla nu e libera, nu putem pune nimic
